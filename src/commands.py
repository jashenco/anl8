...
class ReadLogsCommand(Command):
    def execute(self):
        ...
        """
        try:
            logs = _DBManager.select_all("SELECT * FROM logs")
            for log in logs:
                print(log)
...
class RegisterMemberCommand(Command):
    def execute(self):
        ...
        try:
            ...
            _DBManager.modify("INSERT INTO members (member_id, first_name, last_name, age, gender, weight, address, email, phone, registration_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
class GetMemberDetailsCommand(Command):
    def execute(self):
        ...
        try:
            ...
            member = _DBManager.select("SELECT * FROM members WHERE member_id = ?", (member_id,))
            if member:
                print(member)
...
class SearchMemberCommand(Command):
    def execute(self):
        ...
        """
        try:
            search_key = input("Enter search key (name, email, phone, etc.): ")
            members = _DBManager.select_all("SELECT * FROM members WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ?",
                                            (f"%{search_key}%", f"%{search_key}%", f"%{search_key}%", f"%{search_key}%"))
...
class UpdateMemberCommand(Command):
    def execute(self):
        ...
        try:
            ...
            _DBManager.modify("UPDATE members SET first_name = ?, last_name = ?, age = ?, gender = ?, weight = ?, address = ?, email = ?, phone = ? WHERE member_id = ?",
