...
def display_login():
    ...
    if not username or not password:
        ...
        _Logger.log_activity("System", "Invalid Input", "Invalid username or password input.")
        display_login()

    success = _Authenticator.login(username, password)

    if success:
        _Logger.log_activity(username, "Login Successful", f"User {username} logged in as {_Authorizer.get_current_role()}.")
        print(f"Welcome, {username}! You are logged in as {_Authorizer.get_current_role()}.")
    else:
        _Logger.log_activity(username, "Login Failed", "Login attempt failed.")
    ...
    while True:
        try:
            ...
            if choice in role_options:
                ...
                _Logger.log_activity(_Authorizer.get_current_user()[1], "Menu Selection", f"Selected option: {choice} - {function_name}")
                _CommandFactory.execute_function(function_name)
            else:
                print("Invalid choice. Please try again.")
                _Logger.log_activity(_Authorizer.get_current_user()[1], "Invalid Menu Selection", f"Invalid choice entered: {choice}")
